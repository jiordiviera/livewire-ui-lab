...
codeBlocks.forEach((element, key) => {
    ...
    copyToClipboardContainer.innerHTML = `
        ...
        <div x-show="copyNotification" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-2" x-transition:enter-end="opacity-100 translate-x-0" x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 translate-x-0" x-transition:leave-end="opacity-0 translate-x-2" class="absolute left-0" x-cloak>
            <div class="px-3 h-7 -ml-1.5 items-center flex text-xs bg-primary/50 border-r border-primary/50 -translate-x-full text-foreground rounded">
                <span>Copié!</span>
                <div class="absolute right-0 inline-block h-full -mt-px overflow-hidden translate-x-3 -translate-y-2 top-1/2">
                    <div class="size-3 origin-top-left transform rotate-45 bg-primary/50 border border-transparent"></div>
                </div>
            </div>
        </div>
        <button @click="copyToClipboard();" class="flex items-center justify-center h-8 text-xs bg-gray-700 rounded-md cursor-pointer w-9 hover:bg-gray-900/50 active:bg-gray-600 focus:bg-gray-700 focus:outline-none text-foreground hover:text-foreground/80 group">
            <svg x-show="copyNotification" class="size-4 text-primary/50 stroke-current" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" x-cloak>
